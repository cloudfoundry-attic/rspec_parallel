  ...
  module ReportHelper
    ...
    def get_last_report_file(base_folder, single_report)
      if single_report
        ...
        report_folder = get_report_folder(false)
        return File.join(last_report_folder, "junitResult.xml")
      end
    end
    ...
    def generate_reports(report_folder, time, case_info_list, options)
      ...
      # update a single report or not
      is_update_single_report = options[:rerun] && options[:single_report]
      ...
      elsif not is_update_single_report
