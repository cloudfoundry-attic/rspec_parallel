...
class Rspec_parallel
  ...
  def generate_reports(time, update_report)
    ...
    if File.exists? report_file_path
      fr = File.open(report_file_path, 'a')
    else
  ...
  def generate_single_file_report(case_info_list, is_update)
    ...
    single_output = "<testsuite name=\"#{suite_name}\" tests=\"#{case_info_list.size}\" time=\"#{suite_duration}\" failures=\"#{fail_num}\" errors=\"#{error_num}\" skipped=\"#{pending_num}\">\n"
    ...
    single_output += "<system-out>\n"
    single_output += stdout.encode({:xml => :text}) if stdout.length > 0
    single_output += "</system-out>\n"
    single_output += "<system-err>\n"
    single_output += stderr.encode({:xml => :text}) if stderr.length > 0
    single_output += "</system-err>\n"
    single_output += "</testsuite>\n"

    if is_update == true
      ff = File.open(file_name, 'a')
    else
